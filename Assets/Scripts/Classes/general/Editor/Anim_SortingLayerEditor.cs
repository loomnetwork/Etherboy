...
namespace Anim_Sys
{
	[CustomEditor(typeof(Anim_SortingLayer)), CanEditMultipleObjects]
	public class Anim_SortingLayerEditor : Editor
	{
		...
		private Renderer[] renderers
		{
			get
			{
				if (_renderers == null)
				{
					...
					{
						_renderers[i] = (targets[i] as Anim_SortingLayer).GetComponent<Renderer>();
					}
				}
		...
		void OnEnable()
		{
			...
			var renderer = (target as Anim_SortingLayer).gameObject.GetComponent<Renderer>();
			if (!renderer)
			{
				return;
			}
			...
			if (!(target as Anim_SortingLayer).initialized)
			{
				(target as Anim_SortingLayer).uvsDefault = (target as Anim_SortingLayer).gameObject.GetComponent<MeshFilter>().sharedMesh.uv;
				(target as Anim_SortingLayer).initialized = true;
			}
		}
...

		{
			...
			(target as Anim_SortingLayer).offsetAmount = EditorGUILayout.Vector2Field("Offset UVs", (target as Anim_SortingLayer).offsetAmount);
		...
		public void OffsetUVs()
		{
			...
			Mesh meshToEdit = (target as Anim_SortingLayer).gameObject.GetComponent<MeshFilter>().sharedMesh;

			Vector2[] uvs = new Vector2[meshToEdit.vertexCount];
			for (int i = 0; i < meshToEdit.vertices.Length; i++)
			{
				uvs[i] = (target as Anim_SortingLayer).uvsDefault[i] + (target as Anim_SortingLayer).offsetAmount;
